var assert = require('assert'),
    ...
    // name
    IGNORE = ['_events', 'domain', '_maxListeners', 'pause', 'resume', 'emit'],
    ...
    if ((r = utils.line(b, indata.line_s)) === false)
      ...
      if (indata.expect > -1) {
        ...
        if ((m = /\* (\d+) FETCH/i.exec(indata.line))
            && /^BODY\[/i.test(litType)) {
          var seqno = parseInt(m[1], 10);
          var uid = parseInt(/UID (\d+)/i.exec(indata.line)[1], 10);
          var fetches = requests[0].fetchers[litType.replace(RE_DBLQ, '')];
          for (var i=0, len=fetches.length; i<len; ++i) {
            var currMsg = fetches[i]._msg;
            if (currMsg) {
              if (currMsg.seqno === seqno || currMsg.uid === uid) {
                msg = currMsg;
                break;
              }
            }
          ...
          if (! msg) {
            ...
            if (self._serverSupports('X-GM-EXT-1')) {
              msg['x-gm-thrid'] = /X-GM-THRID (\w+)/.exec(indata.line)[1];
              msg['x-gm-msgid'] = /X-GM-MSGID (\w+)/.exec(indata.line)[1];
              msg['x-gm-labels'] = /X-GM-LABELS \((.+)\)/.exec(indata.line)[1].split(' ').map(function (x) { return x.slice(5, x.length-1); });
            }
              ...
              for (body = 0, lenb = bodies.length; body < lenb; body += 2) {
                ...
                for (var i=0, len=fetches.length; i<len; ++i) {
                  ...
                  for (var k = 0, keys = Object.keys(details), lenk = keys.length;
                       k < lenk; ++k) if (-1 === IGNORE.indexOf(keys[k])) {
...
function ImapMessage() {
  ...
  streamify(this);
  this.pause();
  // TODO 'headers' should not be emitted? 'data' instead
  // when headers.parse === false && body === false
  var self = this;
  this.on('headers', function (headers) {
    self.emit('data', headers);
  });
}
...
function ImapFetch() {
  ...
  this.once('end', function () {
    self.removeListener('message', onMsg);
  });
...
function streamify (stream) {
  Stream.call(stream);
  stream.readable = true;
  var paused = false;
  var buffer = [];
  var emit = stream.emit;
  stream.emit = function (channel, data) {
    if (paused && channel !== 'newListener') buffer.push([channel, data]);
    else {
      emit.call(stream, channel, data);
    }
  };
  ...
  stream.resume = function () {
    ...
    for (var i = 0, l = buffer.length; i < l; i++) {
      stream.emit(buffer[i][0], buffer[i][1]);
    }
    buffer.length = 0;
  };
}
